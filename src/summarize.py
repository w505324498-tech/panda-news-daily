...
def generate_xhs_draft(best_project: dict, ai_news: list[dict]) -> dict:
    ...
    prompt = (
        ...
        "- 账号定位：普通上班族探索AI工具，不是AI专家或开发者\n"
        "- 不像广告，不像AI生成，语气自然口语化\n"
        "- 不要堆砌技术术语\n\n"

        "## ⚠️ 人设约束（非常重要）\n"
        "博主只是发现了这个项目，尚未实际使用过。正文中只能使用以下表达：\n"
        "✅ 可用：「我发现」「我关注到」「最近注意到」「准备试试看」\n"
        "✅ 可用：「看起来」「从介绍来看」「根据文档」「后续继续研究」\n"
        "✅ 可用：「如果好用再继续分享」「先马住」「蹲一个测评」\n"
        "❌ 禁止：「我已经测试过」「我已经使用过」「我搭建成功了」\n"
        "❌ 禁止：「我用了一周」「实测」「亲测」「我跑了demo」\n"
        "❌ 禁止：任何暗示已经动手操作过的表述\n"
        "除非项目确实只需要打开网页就能用（如在线工具），才可以使用「试了一下」\n\n"

        "## 结构模板\n"
        "第一段：发现这个项目的场景（刷GitHub/看别人推荐/偶然看到）\n"
        "第二段：这个项目是干什么的（用大白话解释）\n"
        "第三段：为什么觉得值得关注（解决了什么问题）\n"
        "第四段：准备怎么用/后续计划\n"
        "结尾：轻松收尾，不要催促关注\n\n"

        "## 封面文案要求（cover_texts，3个）\n"
        "- 简短有冲击力，适合做封面大字\n"
        "- 例如：AI工具发现 / 本周AI新发现 / 打工人AI探索\n"
        "- 每条10字以内\n"
        "- 不要包含「实测」等暗示已使用的词\n\n"
